...
exports.getProfile = asyncHandler(async (req, res, next) => {
  ...
  if (!id) return res.redirect("/profile/" + req.session.userId); //render my profiel page
  ...
  if (user) {
    return res.render("user/profile", {
      ...
      isSentRequest: user.friendRequests.find(
        (frind) => frind.id === req.session.userId
      ),
      //4- if i'm recived a friend request from this user
      isRecivedRequest: user.sentRequests.find(
        (frind) => frind.id === req.session.userId
      ),
    });
  } else {
